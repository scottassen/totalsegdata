function GeoFilter3D_NRRD()
clc ; close all
%% Application of Geodesic Filter to a 3D volume in NRRD format to remove  Noise 
% Filter is applied to the volume channel value
% WL,WC,WS number of rows, columns, and depth of the filter
% sigma Sigma value for Gaussian weigths
% alpha Scaling factor for the data channel
%%
% Enter NRRD file name to process
%
prompt = 'Enter Input NRRD file Name? ';
i_file = input(prompt,'s');
headerInfo = nhdr_nrrd_read(i_file, 1);
disp(headerInfo);
sz=headerInfo.sizes;
V=zeros(sz(1),sz(2),sz(3));
for i=1:headerInfo.sizes(1)
    for j=1:headerInfo.sizes(2)
        for k=1:headerInfo.sizes(3)
            V(i,j,k)=headerInfo.data(i,j,k);
        end
    end
end
...
sigma = input(prompt);
%% Visualize input volume data
 volumeViewer(V);
%% Perfom volume filtering
V_fil=Geodesic3DFilter_Vol(V,WL,WC,WS,sigma,alpha);
%% Save Filtered Volume
prompt = 'Enter Output Volume NRRD File Name? ';
o_file = input(prompt,'s');
headerInfo.content=o_file;
headerInfo.dimension=3;
headerInfo.type='float';
headerInfo.kinds={'domain'  'domain'  'domain'};
headerInfo.data=cast(V_fil,'single');
headerInfo.spacedirections_matrix
%%headerInfo.spacedirections={'(1,0,0)' '(0,1,0)' '(0,0,1)'};
headerInfo.sizes=size(V_fil);
headerInfo.encoding='gzip';
headerInfo = nhdr_nrrd_write(o_file, headerInfo, 1);
disp(headerInfo);
%% Display Filtered Volume
volumeViewer(V_fil);
end
